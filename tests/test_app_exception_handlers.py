...
class TestAppExceptionHandlers:
    ...
    def test_pydantic_validation_errors(self):
        ...
        test_app = Litestar(route_handlers=[test_validation_endpoint], debug=True)
        ...
        with TestClient(app=test_app) as client:
            ...
            )  # Litestar returns 400 for validation errors
            ...
            response = client.post("/test-validation", json="invalid_structure")
            assert (
                response.status_code == 400
            )  # Litestar returns 400 for validation errors
    ...
    def test_http_exceptions_are_handled(self):
        ...
        test_app = Litestar(route_handlers=[test_http_exception_endpoint], debug=True)
    ...
    def test_validation_exception_details(self):
        ...
        with TestClient(app=test_app) as client:
            response = client.post(
                "/test-validation",
                json={"required_field": "test", "numeric_field": "invalid"},
            )
    ...
    def test_large_request_handling(self):
        ...
        with TestClient(app=app) as client:
            ...
            response = client.post("/convert", json={"input": {"text": large_text}})
    ...
    def test_empty_request_body(self):
        ...
        with TestClient(app=app) as client:
            response = client.post(
                "/convert", content="", headers={"Content-Type": "application/json"}
            )
            assert response.status_code in [400, HTTP_422_UNPROCESSABLE_ENTITY]
