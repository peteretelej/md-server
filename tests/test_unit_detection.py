...
class TestContentTypeDetector:
    ...
    def test_all_magic_bytes(self):
        ...
        for content, expected_type in test_cases:
            detected = ContentTypeDetector.detect_from_magic_bytes(
                content + b"\x00" * 50
            )
            assert detected == expected_type, (
                f"Failed for {content[:10]}: expected {expected_type}, got {detected}"
            )
    ...
    def test_content_type_header_detection(self):
        ...
        for content_type, expected in test_cases:
            ...
            assert detected == expected, (
                f"Failed for {content_type}: expected {expected}, got {detected}"
            )
    ...
    def test_source_type_mapping(self):
        ...
        for mime_type, expected_source in mappings:
            ...
            assert source == expected_source, (
                f"Failed for {mime_type}: expected {expected_source}, got {source}"
            )
    ...
    def test_supported_format_check(self):
        ...
        for format_type in supported:
            assert ContentTypeDetector.is_supported_format(format_type), (
                f"{format_type} should be supported"
            )

        for format_type in unsupported:
            assert not ContentTypeDetector.is_supported_format(format_type), (
                f"{format_type} should not be supported"
            )
