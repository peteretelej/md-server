...
class TestBrowserLogging:
    ...
    def test_available_browser_logging_message(self, caplog_with_level):
        ...
        assert (
            "URL Conversion: Using Crawl4AI with Playwright browsers" in record.message
        )
    ...
    def test_unavailable_browser_warning_message(self, caplog_with_level):
        ...
        assert (
            "WARNING: URL Conversion: Playwright browsers not available"
            in record.message
        )
    ...
    @pytest.mark.parametrize(
        "available,expected_level,expected_content",
        [
            (True, "INFO", "Using Crawl4AI with Playwright browsers"),
            (False, "WARNING", "Playwright browsers not available"),
        ],
    )
    def test_parametrized_logging(
        self, caplog_with_level, available, expected_level, expected_content
    ):
