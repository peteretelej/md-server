...
class TestMagicBytesComprehensive:
    ...
    def test_all_magic_bytes_detection(self):
        ...
        for content, expected_type in magic_bytes_tests:
            ...
            assert detected == expected_type, f"Failed to detect {expected_type} from {content[:20]}..."
    ...
    def test_office_document_detection(self):
        ...
        assert detected in ["application/zip", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"]
        ...
        assert detected in ["application/zip", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"]
        ...
        assert detected in ["application/zip", "application/vnd.openxmlformats-officedocument.presentationml.presentation"]
    ...
    def test_text_vs_binary_detection(self):
        ...
        for content in text_samples:
            ...
            if detected:
                assert detected.startswith(('text/', 'application/json', 'application/xml'))
    ...
    def test_filename_detection_comprehensive(self):
        ...
        for filename, expected_type in filename_tests:
            ...
            if detected:
                assert detected == expected_type, f"Failed to detect {expected_type} from {filename}"
    ...
    def test_priority_detection(self):
        ...
        detected = ContentTypeDetector.detect_from_content(pdf_content, filename="document.txt")
        assert detected == "application/pdf"
        
        # Filename should be used when no content magic bytes
        detected = ContentTypeDetector.detect_from_content(b"plain text content", filename="document.json")
        ...
        detected = ContentTypeDetector.detect_from_content(b"ambiguous binary content", filename="test.pdf")
    ...
    def test_malformed_inputs(self):
        ...
        assert ContentTypeDetector.detect_from_magic_bytes(None) == "text/plain"  # Default for empty/None
