...
class TestRemoteRetryLogic:
    ...
    def client_no_retries(self):
        ...
        return RemoteMDConverter(
            endpoint="http://127.0.0.1:8999", timeout=2, max_retries=0
        )
    ...
    async def test_custom_retry_configuration(self):
        ...
        client = RemoteMDConverter(
            endpoint="http://127.0.0.1:8999", max_retries=5, retry_delay=0.05
        )
    ...
    async def test_retry_preserves_original_exception(self, client_with_retries):
        ...
        original_error = NetworkError(
            "Original connection failure", {"host": "example.com"}
        )
    ...
    @pytest.mark.asyncio
    async def test_retry_across_all_conversion_methods(
        self, client_with_retries, tmp_path
    ):
