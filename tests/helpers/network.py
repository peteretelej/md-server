...
class NetworkFailureSimulator:
    ...
    def connection_refused(
        ...
        with (
            patch("socket.socket.connect", side_effect=mock_connect),
    ...
    def intermittent_failures(
        self, failure_rate: float = 0.3, max_retries: int = 3
    ) -> Generator[None, None, None]:
    ...
    def corrupt_response(
        self, corruption_rate: float = 0.1
    ) -> Generator[None, None, None]:
    ...
    @contextmanager
    def bandwidth_limit(
        self, bytes_per_second: int = 1024
    ) -> Generator[None, None, None]:
        ...
        class ThrottledResponse:
            def __init__(self):
                ...
                self.headers = {"content-type": "application/json"}
                self._data = (
                    b'{"throttled": "response", "data": "' + b"x" * 10000 + b'"}'
                )
