        ...
        **kwargs
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                last_exception = TimeoutError(f"Request timed out after {self.timeout}s", {"url": url})
                ...
                if any(term in error_msg for term in ["connection", "network", "refused", "unreachable"]):
                    last_exception = NetworkError(f"Network error: {str(e)}", {"url": url})
                else:
                    last_exception = ConversionError(f"Unexpected error: {str(e)}", {"url": url})
                break
        ...
        try:
            ...
            if "error" in error_data:
                ...
                if response.status_code == 400:
                    ...
                    raise ConversionError(f"Server error ({response.status_code}): {message}", details)
            else:
