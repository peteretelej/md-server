...
class FileConverter(BaseConverter):
    ...
    def _validate_file(self, file: UploadFile):
        if file.content_type not in self.settings.allowed_file_types:
            raise UnsupportedFileTypeError(f"File type {file.content_type} not supported")
        
        if file.size and file.size > self.settings.max_file_size:
            raise FileTooLargeError(f"File size {file.size} exceeds limit {self.settings.max_file_size}")