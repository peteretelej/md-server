...
class LLMConfig(BaseModel):
    ...
    client_type: str = Field(..., description="Type of LLM client (openai, azure_openai, anthropic)")
...
class AzureDocIntelConfig(BaseModel):
    ...
    model_id: Optional[str] = Field(None, description="Custom model ID for document analysis")
...
class ConverterConfig(BaseModel):
    ...
    priority: float = Field(0.0, description="Converter priority (lower = higher priority)")
    config: Dict[str, Any] = Field(default_factory=dict, description="Custom converter configuration")
...
class MarkItDownConfig(BaseSettings):
    ...
    # LLM integration
    llm_config: Optional[LLMConfig] = Field(None, description="LLM client configuration")
    
    # Azure Document Intelligence
    azure_docintel_config: Optional[AzureDocIntelConfig] = Field(None, description="Azure Document Intelligence configuration")
    
    # Custom converters
    custom_converters: List[ConverterConfig] = Field(default_factory=list, description="Custom converter configurations")
    
    # Advanced options
    exiftool_path: Optional[str] = Field(None, description="Path to exiftool binary")
    style_map: Optional[Dict[str, str]] = Field(None, description="Custom style mapping for document conversion")
    ...
    requests_user_agent: str = Field("MarkItDown-Server/1.0", description="User-Agent header for requests")
    
    # Plugin system
    plugin_search_paths: List[str] = Field(default_factory=list, description="Additional paths to search for plugins")
    ...
    azure_docintel_endpoint: Optional[str] = Field(None, description="Azure Document Intelligence endpoint")
    azure_docintel_key: Optional[str] = Field(None, description="Azure Document Intelligence API key")
    ...
    def validate_custom_converters(cls, v):
        ...
        for converter in v:
            if not converter.module_path or not converter.class_name:
                raise ValueError("Custom converters must have module_path and class_name")
        return v
        ...
        try:
            if self.llm_config.client_type == "openai":
                ...
                return AzureOpenAI(
                    ...
                    azure_endpoint=self.llm_config.base_url or self.azure_openai_endpoint,
    ...
    def get_azure_docintel_credential(self) -> Optional[Any]:
        ...
        if api_key:
            try:
                ...
                raise ValueError("Azure libraries not installed for Document Intelligence")
    ...
    def load_custom_converters(self) -> List[Any]:
        ...
        for converter_config in self.custom_converters:
            try:
                ...
                raise ValueError(f"Failed to load converter {converter_config.class_name}: {e}")
