...
class ProcessingStats:
    ...
    def success_rate(self) -> float:
        return (
            (self.successful / self.total_files * 100) if self.total_files > 0 else 0.0
        )
...
async def batch_file_processing():
    ...
    converter = MDConverter(ocr_enabled=True, timeout=60, max_file_size_mb=100)
    ...
    async def process_single_file(
        file_path: str,
    ) -> tuple[str, ConversionResult | Exception]:
    ...
    async def smart_convert_url(url: str) -> ConversionResult:
        ...
        try:
            result = await converter.convert_url(
                url, js_rendering=needs_js, extract_images=True
            )
...
async def content_pipeline():
    ...
    class ContentPipeline:
        ...
        def __init__(self):
            self.converter = MDConverter(clean_markdown=True, preserve_formatting=True)
    ...
    class MonitoredConverter:
        ...
        async def convert_with_monitoring(self, file_path: str) -> ConversionResult:
            ...
            try:
                ...
                # Log error
                logger.error(
                    f"Conversion failed: {file_path} - {e} (after {duration:.2f}s)"
                )
                raise
        ...
        def get_metrics_report(self) -> Dict[str, Any]:
            ...
            return {
                "total_conversions": self.metrics["successful_conversions"]
                + self.metrics["failed_conversions"],
